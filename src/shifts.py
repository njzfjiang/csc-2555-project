...
def group_shift(severity=0.5):
    ...
    X_shifted, y, s = generate_data(base_rate_a=new_base_rate_a, base_rate_b=new_base_rate_b)
    
    return X_shifted, y, s

def covariate_shift(severity=1.0, group='A'):
    """
    Apply a covariate shift to the feature 1 of Group A.
    ...
    X_shifted, y, s = generate_data(cov_scale_a=cov_a, cov_scale_b=cov_b)
    return X_shifted, y, s

def label_shift(severity=0.1):
    """
    ...
    X, y_shifted, s = generate_data(flip_noise_a=severity)
