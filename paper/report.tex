...
\textbf{Measurement distortion (RQ2).} Under asymmetric label noise, DP remains 0.014 and predictions and $Y^*$ do not change, yet the recorded-label TPR gap ends at 0.150 versus 0.034 without noise. The apparent degradation therefore arises entirely from evaluating against $\tilde{Y}$ rather than a change in clean-outcome performance.
...
\]
The rule requires a TPR-gap increase of at least five percentage points and either no more than a two-point DP increase or a TPR-gap increase at least twice the nonnegative DP increase. These are monitoring cutoffs, not clinical standards or statistical-significance tests (Appendix~\ref{app:d}). A cell is marked when at least three of five seeds satisfy the rule. Figure~\ref{fig:danger_zone} shows 20 dangerous cells out of 110, emerging from $\beta\approx0.17$ at lower $\alpha$. Recorded and clean TPR gaps differ by at least five percentage points in 78 cells; their maximum increases are 0.338 and 0.021, respectively. One-at-a-time sensitivity checks retain nonempty danger zones (12--31 cells), with the footprint most sensitive to the cross-metric multiplier (Table~\ref{tab:danger_sensitivity}).
...
\noindent\textbf{Interpretation.} The danger zone begins at $\beta=0.167$ for $\alpha\le0.1$ and expands to $\alpha\le0.4$. DP can appear stable or improve while recorded-label TPR disparity worsens; 78 cells show clean--recorded disagreement beyond the 20-cell zone. Thus, conclusions depend on both metric and outcome definition.

\section{Conclusion and Future Work}

Stable DP can mask recorded-label disparities, while documentation noise can create apparent disparity. Monitoring should tie metrics and data quality to explicit outcomes and respect each mitigation's access and shift assumptions. Future work should study richer features and causal outcome definitions.
...
\bibitem{liao2022social}
Liao, Y., \& Naghizadeh, P. (2022). Social bias meets data bias: The impacts of labeling and
measurement errors on fairness criteria. arXiv preprint arXiv:2206.00137.
...
Finally, \texttt{label\_shift} modifies recorded labels asymmetrically across groups. Its \texttt{severity} argument, chosen in \([0, 0.3]\), controls the probability with which clean positive labels for group A and, at half that rate, clean negative labels for group B are flipped. Concretely, we call
\begin{itemize}
